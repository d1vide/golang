...
type Handler struct {
	...
	auth   *authgrpc.Client
	logger *slog.Logger
}

func NewHandler(svc *service.TasksService, auth *authgrpc.Client) *Handler {
...
func (h *Handler) authGuard(w http.ResponseWriter, r *http.Request) bool {
	...
	err := h.auth.Verify(r.Context(), authHeader)
	if err == nil {
		return true
	}

	if errors.Is(err, authgrpc.ErrUnauthorized) {
